...
- 업로드 중에는 진행 상태를 확인할 수 있습니다.
- 업로드된 문서는 내역서, 도면, 시방서별로 따로 정리됩니다.
- 잘못 올린 문서는 삭제할 수 있습니다.
- AI 검토 실행 시 사용할 문서를 직접 선택할 수 있습니다.
- 전체 문서를 한 번에 선택하거나, 필요한 문서만 골라 검토할 수 있습니다.
...
- 추출 결과는 기본적으로 요약 상태로 표시되고, 필요할 때만 원문 추출 내용을 펼쳐서 확인할 수 있습니다.
- 추출 결과는 전체 문서 기준 또는 AI 검토 대상으로 선택한 문서 기준으로 나누어 볼 수 있습니다.
- 추출 결과 화면에서도 잘못 올린 문서를 삭제할 수 있습니다.
...
- AI 응답 형식이 일부 흔들려도 전체 검토가 멈추지 않도록, 가능한 결과와 규칙 기반 후보를 함께 반영합니다.
- AI 요청 한도를 넘는 경우 일부 문서는 축소 검토하거나, 공종 또는 문서 범위를 좁혀 다시 검토하도록 안내합니다.
...
- 결과 조건에서는 파일명 필터를 제외하고, 실제 검토 흐름에 필요한 일자, 공종, 유형만 사용합니다.
- 항목을 선택하면 상세 내용, 확인 위치, 권장 조치, RFI 초안을 볼 수 있습니다.
- RFI 초안은 화면에서 복사할 수 있습니다.
...
- RFI 후보가 여러 개면 한 문서 안에 여러 건을 나누어 작성합니다.
- 양식의 칸은 RFI 항목 수에 맞춰 반복 생성됩니다.
- 생성된 RFI 문서는 저장소에 보관됩니다.
- 이후에도 생성된 RFI 문서 목록에서 다시 다운로드하거나 삭제할 수 있습니다.
...
- 상단 `CONMA` 브랜드 바
- 큰 콘마 아이콘과 짧은 서비스 설명
- 브랜드 첫 화면과 1~6단계 설명 화면 분리
- 스크롤에 따라 바뀌는 1~6단계 설명
- 오른쪽 단계 점 표시
- 단계 점을 선택하면 해당 설명 위치로 이동
- `SCROLL · STORY` 안내
...
작업 화면에서는 왼쪽 사이드바와 상단 프로젝트 빠른 선택칸을 사용합니다.

이렇게 구성한 이유는 작업 중에는 메뉴와 현재 프로젝트가 항상 보여야 이동이 편하기 때문입니다.
...
- **RFI Word 양식**: 검토 결과를 공식 질의서 형태의 `.docx` 파일로 생성하는 기준 양식
...
- 큰 문서는 한도 초과를 막기 위해 일부 내용을 압축하거나 제외할 수 있습니다.
...
- RFI 문서 목록에서 다운로드와 삭제 가능
- 문서 업로드와 추출 결과를 내역서, 도면, 시방서별로 정리
- 추출 결과를 전체 문서와 검토 대상 문서 기준으로 필터링
- AI 검토 전 사용할 문서를 직접 선택
- 큰 문서 AI 검토 시 자동 분할, 압축, 재시도 로직 추가
- AI 응답 오류가 일부 발생해도 전체 결과가 중단되지 않도록 보완
